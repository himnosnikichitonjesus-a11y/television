// =============================================================
// Watch.js — Reproductor principal con controles modernos
// Modo normal: auto‑ocultado por inactividad, cola, recomendaciones
// =============================================================
...
const ICONS = {
  ...
  vol:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3A4.5 4.5 0 0 0 14 8.04v7.92A4.5 4.5 0 0 0 16.5 12zM14 3.23v2.06A7 7 0 0 1 14 18.7v2.06A9 9 0 0 0 14 3.23z"/></svg>',
  mute:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.21.05-.42.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51A8.8 8.8 0 0 0 21 12a9 9 0 0 0-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.17v2.06a8.99 8.99 0 0 0 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>',
  full:
  ...
  pip:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 7h-8v6h8V7zm2-4H3a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h18a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2zm0 16.01H3V4.98h18v14.03z"/></svg>',
  settings:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M19.43 12.98c.04-.32.07-.64.07-.98s-.03-.66-.07-.98l2.11-1.65a.5.5 0 0 0 .12-.64l-2-3.46a.5.5 0 0 0-.61-.22l-2.49 1a7.3 7.3 0 0 0-1.69-.98l-.38-2.65A.49.49 0 0 0 14 2h-4a.49.49 0 0 0-.49.42l-.38 2.65c-.61.25-1.17.57-1.69.98l-2.49-1a.5.5 0 0 0-.61.22l-2 3.46a.5.5 0 0 0 .12.64l2.11 1.65c-.04.32-.07.65-.07.98s.03.66.07.98l-2.11 1.65a.5.5 0 0 0-.12.64l2 3.46c.14.24.43.34.68.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.04.24.25.42.49.42h4c.24 0 .45-.18.49-.42l.38-2.65c.61-.25 1.17-.57 1.69-.98l2.49 1c.25.1.55 0 .68-.22l2-3.46a.5.5 0 0 0-.12-.64l-2.11-1.65zM12 15.5A3.5 3.5 0 1 1 12 8.5a3.5 3.5 0 0 1 0 7z"/></svg>',
  queue:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M3 10h11v2H3zM3 6h11v2H3zM3 14h7v2H3zM16 13v8l7-4z"/></svg>'
};
...
const IMG_ICONS = {
  ...
  modeVideo:
    '<img src="https://nikichitonjesus.odoo.com/web/image/1110-40385f0d/video.webp" alt="video" style="width:100%;height:100%;object-fit:contain;">',
  modeAudio:
    '<img src="https://nikichitonjesus.odoo.com/web/image/625-e42b8a86/audio.png" alt="audio" style="width:100%;height:100%;object-fit:contain;">'
};

// ========== ESTADO GLOBAL ==========
let active = null; // reproductor activo
...
function injectWatchStyles() {
  ...
  style.textContent = `
    ...
    .player-area {
      ...
      background: #0a0a0a;
      width: 100%;
      aspect-ratio: 16 / 9; /* se mantiene la relación */
    }
    .media-host {
      width: 100%;
      height: 100%;
    ...
    .center-controls .ctrl-center:hover {
      transform: scale(1.08);
      background: rgba(255, 255, 255, 0.16);
      box-shadow: 0 12px 40px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.25);
    }
    .center-controls .ctrl-center:active {
    ...
    .series-list .side-ep-item {
      background: rgba(255,255,255,0.04);
    }

    /* Modal y Toast (compartidos) */
    .nk-modal-overlay {
      position: fixed;
      top: 0;
      left: 0;
  ...
  return `
    <div class="player-controls" id="player-controls">
      ...
      <div class="controls-row">
        ...
        ${canSwitch ? `<button class="ctrl" data-act="switch-mode" id="btn-mode" title="Cambiar audio/video">${IMG_ICONS.modeVideo}</button>` : ''}
...
function setupPlayer(root, media, ep, queue, queueIndex, ctx, initialMode) {
  ...
  // --- Ocultar/Mostrar controles (normal) ---
  const hideControls = () => {
    const ctrl = root.querySelector('.player-controls');
    if (ctrl) ctrl.classList.add('hide-controls');
    if (centerControls) centerControls.classList.add('hide-controls');
  };
  const showControls = () => {
    const ctrl = root.querySelector('.player-controls');
    if (ctrl) ctrl.classList.remove('hide-controls');
  ...
  root.querySelectorAll('[data-act]').forEach((b) => {
    b.addEventListener('click', (e) => {
      ...
      switch (act) {
        ...
        case 'toggle-queue':
          toggleSidePanel(side, sideInner, queue, ep, ctx);
          break;
        case 'minimize':
          ctx.minimizeToPip?.();
          ctx.navigate('/');
...
function switchMode(area, ep, newMode, oldMedia, ctx) {
  ...
  // Actualizar referencia en active
...
// ========== PANEL LATERAL (lista de episodios) ==========
function toggleSidePanel(side, inner, queue, ep, ctx) {
